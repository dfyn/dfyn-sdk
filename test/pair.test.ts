...
describe('Pair', () => {
  ...
  describe('constructor', () => {
    it('cannot be used for tokens on different chains', () => {
      expect(() => new Pair(new TokenAmount(ETH, '100'), new TokenAmount(WETH[ChainId.OKEX], '100'))).toThrow(
        'CHAIN_IDS'
      )
    })
  })
  ...
  describe('#token1', () => {
    it('always is the token that sorts after', () => {
      expect(new Pair(new TokenAmount(ETH, '100'), new TokenAmount(DFYN, '100')).token1).toEqual(DFYN)
      expect(new Pair(new TokenAmount(DFYN, '100'), new TokenAmount(ETH, '100')).token1).toEqual(DFYN)
    })
  })
  describe('#reserve0', () => {
  ...
  describe('#priceOf', () => {
    ...
    it('returns price of token in terms of other token', () => {
      ...
      expect(pair.priceOf(DFYN)).toEqual(pair.token1Price)
    })
