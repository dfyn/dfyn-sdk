...
export type RemoveAndAddLiquidityOptions = AddLiquidityOptions & RemoveLiquidityOptions
...
export abstract class ConcentratedPoolManager {
  ...
  */
  ...
  /**
   * Produces the calldata for removing a position and creating a new position
   * @param position The position to exit
   * @param options Additional information necessary for generating the calldata
   * @returns The call parameters
   */
  public static removeAndAddCallParameters(positionToBeRemoved: Position, positionToBeAdded: Position, options: RemoveAndAddLiquidityOptions): MethodParameters {
    const removeCallParameters = this.removeCallParameters(positionToBeRemoved, {
      liquidityPercentage: options.liquidityPercentage,
      recipient: options.recipient,
      slippageTolerance: options.slippageTolerance,
      tokenId: options.tokenId,
      unwrapVault: options.unwrapVault
    })

    const addCallParameters = this.addCallParameters(positionToBeAdded, {
      deadline: options.deadline,
      lowerOldTick: options.lowerOldTick,
      recipient: options.recipient,
      slippageTolerance: options.slippageTolerance,
      tokenId: "0",
      upperOldTick: options.upperOldTick,
      useNative: options.useNative,
    })
    return {
      calldata: Multicall.encodeMulticall([removeCallParameters.calldata, addCallParameters.calldata]),
      value: addCallParameters.value
    }
  }
}