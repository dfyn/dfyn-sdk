...
import { BigintIsh, Price, TokenAmount,FullMath, TickMath,TradeType, tickToPrice } from '../index'
 ...
 */
 export class Order {
  ...
  //  if exactIn return new TokenAmount(   this.pool.token0,   this.amountIn.toString() )
  //  else
  //  if 
  //  zeroForOne = false
    ...
    public get tokenAmountIn(): TokenAmount {
      if (this._tokenAmountIn === null) {
        if(this.tradeType===TradeType.EXACT_INPUT)
        {
          ...
          if (!this.zeroForOne) {// 1 -> 0 
            let amountIn = FullMath.mulDivRoundingUp(this.amount, this.sqrtpriceX96, Q96);
            ...
            let amountIn = FullMath.mulDivRoundingUp(this.amount, Q96,
                this.sqrtpriceX96);
            amountIn = FullMath.mulDivRoundingUp(amountIn, Q96,
                this.sqrtpriceX96);
            this._tokenAmountIn = new TokenAmount(
                this.pool.token0, 
                amountIn.toString()
  ...
  public get tokenAmountOut(): TokenAmount {
    if (this._tokenAmountOut === null) {
      if(this.tradeType===TradeType.EXACT_OUTPUT)
        {
          if (this.zeroForOne) {
            this._tokenAmountOut = new TokenAmount(
              this.pool.token1,
              this.amount.toString()
            )
          } else {
      ...
      if (this.zeroForOne) {// 0->1
        let amountOut = FullMath.mulDivRoundingUp(this.amount, this.sqrtpriceX96, Q96);
        amountOut = FullMath.mulDivRoundingUp(amountOut, this.sqrtpriceX96, Q96);        
        this._tokenAmountOut = new TokenAmount(
          this.pool.token1,
          amountOut.toString()
        ...
        let amountOut = FullMath.mulDivRoundingUp(this.amount, Q96,
            this.sqrtpriceX96);
        amountOut = FullMath.mulDivRoundingUp(amountOut, Q96,
            this.sqrtpriceX96);
        this._tokenAmountOut = new TokenAmount(
   ...
   /**
   * Returns the expected amountIn of limit order
   */
